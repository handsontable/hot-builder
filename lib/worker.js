...
Worker.prototype.buildDependencyTree = function() {
  ...
  // include all founded modules except that excluded by user
  if (this.options.buildMode.length) {
    filteredModules = modules.filter(function(module) {
      var includeModule = false;

      if (_this.options.buildMode.indexOf(module.type) > -1 || _this.options.buildMode.indexOf('all') > -1) {
        includeModule = true;
  ...
  // include modules manually
    ...
    filteredModules = modules.filter(function(module) {
      return _this.options.include.indexOf(module.name) !== -1;
    });
  }
  ...
  modules.forEach(function(module, index) {
    ...
    // TODO: Hide other modules then plugin because other modules is WIP
