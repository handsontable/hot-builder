...
Builder.prototype.init = function(argv) {
  ...
  try {
    ...
    // for npm 3.*
    hotDefaultPath = path.join(__dirname, '..', '..', 'handsontable', 'package.json');
  }
...
Builder.prototype.buildExternalModulesDescriptor = function(external) {
  ...
  if (this.options.cli && this.defaultInput) {
    npmPath = npmPath.replace(path.join('node_modules', 'handsontable'), '');
  }
  // Support for npm 3.*
  result.push({
    type: 'dir',
  ...
  result.push({
    type: 'dir',
    path: path.resolve(path.join(__dirname, '..', 'node_modules', 'handsontable', 'node_modules') + '/@(' + requireNpmModules + ')'),
    external: true,
    npm: true,
    modulesDescriptor: this.options.npmModulesDescriptor
  });
