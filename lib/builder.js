...
function Builder(argv, options) {
  ...
  options.baseSrc = options.baseSrc|| 'node_modules/handsontable/src/';
  options.input = options.input || argv.input || 'node_modules/handsontable/package.json';
...
Builder.prototype.getEntryFile = function(entryPoint, outputDir) {
  ...
  if (/package.json$/.test(entryPoint)) {
    content = fs.readFileSync(path.resolve(entryPoint)).toString();
    content = JSON.parse(content);
    file.src = path.resolve(path.normalize(path.dirname(entryPoint) + '/' + content.browser));
    replacer.setVariables(content);
  }
