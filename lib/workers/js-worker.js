...
JSWorker.prototype.run = function(entryFile, callback) {
  ...
  this.b = browserify({
    entries: EntryFile.TYPES.map(function(type) {
  ...
  // TODO: Actually numeral must be forced included into package because core.js use it
  this.b.require(path.join(baseFile.path, 'lib', 'numeral', 'numeral.js'), {
    expose: 'numeral'
  });
  ...
  intro = fs.readFileSync(path.join(baseFile.srcPath, 'intro.js')).toString('utf8');
